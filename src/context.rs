...
impl<'a> Context<'a> {
    ...
    pub fn new_with_shell_and_path(
        ...
        // Canonicalize the current path to resolve symlinks, tildes, etc.
        // NOTE: On Windows this converts the path to extended-path syntax.
        let current_dir = path
            .canonicalize()
            // If canonicalization fails, fall back to expanding the home directory tilde manually.
            .unwrap_or_else(|_| Context::expand_tilde(path));
